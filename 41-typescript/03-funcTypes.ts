...
print(2, 4, 6); // 2 4 undefined
// print(2); // error

//js는 선언된 매개변수의 개수와 함수 호출시 전달되는 파라미터 개수가 달라도 상관없음
//ts는 매우 상관이 있습니다.
...
//#10. 오버라이딩 vs 오버로딩
// 오버라이딩
// 클래스에서 부모 클래스의 메서드를 자시기 클래스에서 재정의하는 개념
...
//오버 로딩
//"동일한 함수 이름"을 사용하지만 서로 다른 매개변수 목록을 가지는 여러 함수 정의를 제공하는
...
//###################################
//빨간 밑줄 사라지도록 함수를 완성하기

function printSome<T1, T2>(x: T1, y: T2) {
  console.log(x, y);
}

printSome<string, string>("hi", "hello");
printSome<number, number>(100, 200);
printSome<[true, false], [false, false]>([true, false], [false, false]);
