...
//rhf 라이브러리
//-코드의 간결화
//-비제어 컴포넌트 / 필요한 경우에만 리렌더링한다.
// 다양한 검증 규칙 쉽게 설정
// handleSubmit 요 함수 하나로 모든 폼 데이터를 쉽게 관리
// 복잡하고 대규모 폼 양식을 작성할 때 사용

// q) why ...을 사용하는가? 
// 리액트 컴포넌트에서 ...연산자는 객체나 배열을 펼쳐서 각각의 요소나 속성을 개별적인 prop으로 전달할 때 사용됩니다.
//이 문법을 사용하면 코드가 간결해지고, 여러 개의 props를 한번에 전달할 수 있습니다.

/*
props ? - 컴포넌트의 전달되는 입력값
        - 부모 컴포넌트가 자식 컴포넌트에 데이터를 전달 할 때 사용되며, 
        - 이 props을 통해 컴포넌트의 동작이나 출력이 결정된다.

react에서는 객체의 속성을 컴포넌트의 props로 전달할 때, 각 속성을 개별적으로 전달 할 수 있습니다.
*/
...
{/* <MyComponent name = {person.name} hobby = {person.game}/> */}

// 스프레드 연산자를 사용하여 전달

{/* <MyComponent {...person}/> */}

//register 함수는 입력 필드를 폼 상태에 등록하고 제어하는데 필요한 여러 속성을 포함하는 객체를 반환합니다. 
