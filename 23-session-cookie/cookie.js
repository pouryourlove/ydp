...
const cookieConfig = {
    ...
    signed: true, // 쿠키의 암호화를 하겠다라는 뜻 (req.signedCookies)에 들어
...
app.get('/setCookie', (req, res) => {
    //res.cookie (쿠키 만드는건 서버측이므로 키,값, 옵션을 넣어주면 됨)
...
app.get('/clearCookie', (req, res) => {
    ...
    //: 쿠키를 생성할 때의 키, 값, 옵션이 일치해야함(단, maxAge/expire 키는 일치할 필요 없음)
...
// -secretKey : 비밀키를 넣을 수 있음. 비밀키의 값을 통해서 해당 쿠키가 이 서버가 만든 쿠키임을 검증. 쿠키는 클라이언트에 저장되는 정보이다 보니 위조가 쉬우므로
// 비밀키를 통해 만든 서명을 쿠키 뒤에 붙임
// -optionObj: 쿠키에 사용되는 옵션 (3일 유지 이런거)
