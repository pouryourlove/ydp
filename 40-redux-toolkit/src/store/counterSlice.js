...
//redux toolkit은 라이브러리를 사용하여 상태를 직접 변경하는 것처럼 보이지만,
...
//첫번째로 리듀서 만들어줬으니 이제 스토어 만들 차례! store 폴더에 index.js 만들어 관리해줌
//createSlice 로 생성한 counterSlice 객체는 name, actions, reducer등을 포함.
//counterSlice.reducer는 createSlice에 의해 자동으로 생성된 리듀서 함수 .
//상태 업데이트를 담당 함. 핵심 함수이기 때문에 default로 내보내는것.
//이름에 s를 붙이지 않고 단수형 reducer를 사용하는 이유는, 슬라이스에서는 단일 리듀서 함수를
//반환하기 때문이다.
