...
//(레인 보우 테이블에서도 암호화된 output을 역추적해서 찾아낼 수도 있음)
...
// 단방향 암호화 생성 함수
// saltAndHashPw: 임의의 salt 값을 생성한 후, pbkdf2Sync 함수를 사용해서 해당 솔트와 비밀번호를 기반으로 해시를 생성
const saltAndHashPw = (pw) => {
    //64개의 가능한 문자로 데이터를 인코딩 하는 방식
    ...
    .toString('base64'); // 암호화된 buffer 형식의 데이터를 base64 문자열로 변환해서 저장하거나 전송하기 쉽도록 할것임
...
const comparePw = (inputPw, savedSalt, savedHash) => {
    ...
    //hash: 사용자가 주장하는 비밀번호를 savedSalt와 조합해서 암호화한 해시값
    //savedHash : 정답 비밀번호에 대한 해시 값

    return hash === savedHash // 두 값이 같으면 true, 다르면 false
}
