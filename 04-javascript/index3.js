...
var a2 =5; 
console.log(a2) //중복 선언 가능

///////////////////////////////////
//** var 변수의 문제점
//1.중간의 같은 이름의 변수를 다시 선언해도 기존의 변수에 덮어 씌움.
// - 변수를 선언 했다는 것은 분명히 다른 데이터를 넣으려는 것인데 그것을 기존의 데이터
    //덮어 씌우면 문제가 발생할 수 있다.
//2. 변수가 (블록 단위)에서 끝나는 것이 아니라 자기 마음대로 전역으로 돌아다니면서
    // 영향력을 행사한다. -> 의도치 않는 문제가 발생한다. 
//3, 호이스팅 문제
// 따라서 es6 문법 부터는 var 대신 let 사용을 권장한다. 

//q.es6가 뭔가요?
//a.js 표준 버전 중 하나이다.(js 주요 업데이트)
...
var x; //변수 선언(호이스팅) - 맨 위에 입력하지 않았는데 엔진 스스로가 변수 선언해버리는거
console.log(x); //undefined
var x = 10;
console.log(x); //10

//변수 x가 선언되기 전에 사용되었지만, js 엔진은 선언을 최상단으로 올려서 undefined를 출력한 후,
//나중에 변수를 초기화 하여 값을 할당. 
...
//-키워드(예약어) 사용 불가능
//키워드란? 
//특별한 역할을 이미 하고 있는 단어. (이미 예약되어 있는 곳을 사용 못하는 것과 같음)
...
//관례: calmelCase
//e.g. redApple, isWater
//snake_case
//ex) red_apple, is_water

// q) 상수란? 
// a) 프로그램이 실행 되는 동안 값이 변하지 않는 데이터를 나타냅니다.
// 한번 정의되면 프로그램 실행 중에는 변경되지 않아야함(고정된 값)
