...
-- 외래키 제약조건
-- 두 테이블 사이에 관계를 맺음 -> 데이터의 무결성(데이터가 이상하지 않고 올바르다는 확정지어줌)
-- 다른 테이블의 기본키를 이 테이블의 외래키로 연결

-- 용어
-- 기존 테이블: 기본키를 갖는 테이블 (customer) 
-- 참조 테이블: 외래키가 있는 테이블 (orders)
-- 외래키 연결: foreign key (참조 테이블 _열이름) references 기준테이블(열이름)
-- on update cascade : 기준 테이블의 데이터가 변경되면 참조 테이블의 데이터도 변경된다 
-- on delete cascade : 기준 테이블의 데이터가 삭제되면 참조 테이블의 데이터도 삭제된다

-- 테이블끼리 관계를 맺고 나면 테이블 삭제할 때 순서에 주의
-- customer.cusid, orders.custid 가 관계를 맺음
-- customer에 존재하는 회원만 orders 테이블에 데이터를 추가 가능
-- orders 테이블이 존재하고 있는 상황에서 customer 테이블을 삭제(drop)하면?
-- orders 테이블은 어떤 고객의 생일 정보를 알고 싶어도 customer 테이블 자체가 날라갔기 때문에 알 수 없다.
-- pk-fk 연결된 테이블은 외래키가 설정된 테이블(참조테이블)이 먼저 삭제되어야 한다.
-- (1) orders 테이블 삭제 -> (2) customer 테이블 삭제의 순서로 삭제해야함.

-- [insert]
insert into customer (custid, cusname, addr, phone, birth)
	values ('lucky', '강해원', '미국 뉴욕', '01022223333', '2002-03-05');

insert into customer (addr, phone, birth, custid, cusname)
	values ('대한민국 부산', '01098765432', '2007-04-28', 'wow', '이지은');
-- 필드명이 명시하지 않는 경우 컬럼이 정의된 순서대로 값을 넣기
 insert into customer
		value ('happy','최시은','일본 오키나와','01033331234','1970-10-31');
...
-- 1. 비교
-- 강해린 고객의 생일 조회
select birth from customer where cusname = '강해린'; -- 작은 따옴표 써야한다.

-- 강해린 고객을 제외한 나머지 고객의 생일 조회
select birth from customer where cusname != '강해린'; 

-- 사전 순으로 박민지 고객보다 뒤에 위치한 고객을 조회
select * from customer where customer > '박민지';

-- 2. 범위
-- 1995 ~ 2000 년 출생한 고객 조회
...
select * from customer where cusname like '_지%';
select * from customer where cusname like '_지'; -- 검색결과:0
-- 고객 이름 세번째 글자가 '수'인 고객
select * from customer where cusname like '__수%';
select * from customer where cusname like '%수'; -- x -> 예외: 지수, 가나다라수

-- 5. 복합 조건

-- 주소지가 대한민국이고, 2000년대 생 이후 출생 고객 검색(and)
select* from customer where addr like '대한민국%' and birth >= '2001-01-01';
-- 주소지가 미국이거나 영국인 고객 검색 (or)
select * from customer where addr like '미국%' or addr like '영국%';
-- 휴대폰 번호 마지막 자리가 4가 아닌 고객 검색(not)
select * from customer where phone not like '%_4'; -- %4 이렇게 하면 안됨. 이유: $4: 마지막 글자가 4,14,1004, ... %_4: 마지막 글자가 14,1004, ... 

-- 6. order by:정렬
select * from customer order by cusname; -- default asc
select * from customer order by cusname desc;

-- 주의) Order by 절은 wehre 절 보다 뒤에 위치해야함
-- 2000년생 이후 출생자 중에서 주소를 기준으로 내림차순 검색
select * from customer where birth >= '2000-01-01' order by addr desc;
-- order by를 먼저 쓰면 구문 순서 오류가 난다. 
...
-- 7. limit : 개수 제한
-- 2000년생 이후 출생자 중에서 주소를 기준으로 내림차순, 그리고 아이디를 기준으로 내림차순 검색하는데 최초의 4개만 보고 싶음
